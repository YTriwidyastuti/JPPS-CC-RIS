...
HRD_csv_train = pd.read_csv('channel\HRD_GMM_S3_R8_M4_N4_U3.csv')
...
# HRD_csv_test = pd.read_csv('HRD_S3_R6_M4_N8_U3_test.csv')
# HRD_data_test = th.tensor(HRD_csv_test.values, dtype=th.double)
# data_len = int(len(HRD_data_test)/U)

# HRD_real = HRD_data_test[:,0:N] * th.cos(HRD_data_test[:,N:2*N])
# HRD_imag = HRD_data_test[:,0:N] * th.sin(HRD_data_test[:,N:2*N])
# HRD = th.complex(HRD_real, HRD_imag).to(device)
# num_batch = int(data_len/train_bsz)
# HRD_batch = HRD.reshape(data_len,U,N)
# HRD_test = HRD_batch.reshape(num_batch,train_bsz,U,N)

# HRD_data_test[:,0:N] = HRD_data_test[:,0:N] * (10**5)
# HRD_data_test[:,N:2*N] = (HRD_data_test[:,N:2*N] + math.pi) / (2*math.pi)
# dataset = HRD_data_test.reshape(data_len,1,U,2*N)
...
init_bsz = 1
out_csv = pd.read_csv('channel\OUT_GMM_S3_R8_M4_N4_U3.csv')
...
for c in range(num_pred):
    ...
    for epoch in range(binit):
        ...
        for xb, yb in suploader:
            ...
            init_SNR1 = init_sr1a / (init_sr1b + init_sr1c + sigma2)
            ...
            init_SNR2 = init_sr2a / (init_sr2b + init_sr2c + sigma2)
            ...
            init_SNR3 = init_sr3a / (init_sr3b + init_sr3c + sigma2)
    ...
    for a in range(epoch_size):
        ...
        for b, train_inp in enumerate(train_dataloader):
            ...
            train_SNR1 = train_sr1a / (train_sr1b + train_sr1c + sigma2)
            ...
            train_SNR2 = train_sr2a / (train_sr2b + train_sr2c + sigma2)
            ...
            train_SNR3 = train_sr3a / (train_sr3b + train_sr3c + sigma2)
...
#         for test_inp in test_dataloader:
            ...
            test_SNR1 = test_sr1a / (test_sr1b + test_sr1c + sigma2)
            ...
            test_SNR2 = test_sr2a / (test_sr2b + test_sr2c + sigma2)
            ...
            test_SNR3 = test_sr3a / (test_sr3b + test_sr3c + sigma2)
...
np.savetxt("out_GMM_S3_R8_M4_N4_U3_250923.csv", out_data, delimiter=",")
